...
Project aim: patching `obsidian_to_anki.py` to work with Zim Wiki. Patches may be breaking and upstreaming isn't currently a high priority.

Current status:

* AnkiCard IDs can now be generated by a Zim plugin/custom tool, thus edits to cards are synced with Anki
* Notes need to be exported to md if they're richer than plain text
